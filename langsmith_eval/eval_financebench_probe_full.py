...
DATASET_NAME = os.getenv("FINANCEBENCH_EVAL_DATASET", "financebench_probe10_v1")

EXPERIMENT_PREFIX = os.getenv(
    "FINANCEBENCH_EXPERIMENT_PREFIX",
    "FinanceBench Probe10 PageIndex EvidencePack",
)

# target 调用方式：
# - import: 直接 import backend/agent.py 的 chat_with_agent
# - http: 调用已经启动的 FastAPI 后端，避免重复加载模型
RAG_TARGET_MODE = os.getenv("RAG_TARGET_MODE", "import").lower()

# 如果用 http 模式，改成你 /docs 里真实的聊天接口
RAG_API_URL = os.getenv("RAG_API_URL", "http://127.0.0.1:8000/chat")

MAX_CONCURRENCY = int(os.getenv("FINANCEBENCH_EVAL_MAX_CONCURRENCY", "1"))
...
# ============================================================
# 3. 从 FinanceBench metadata 中抽取 evidence 字段
# ============================================================
...
# ============================================================
# 4. 从 rag_trace 中抽取 retrieved chunks / evidence pack
# ============================================================

def _extract_retrieved_chunks(outputs: dict) -> list[dict]:
    """
    优先读取新版 final_evidence_pack / selected_pages，
    其次兼容旧版 final_retrieved_chunks / retrieved_chunks。
    """
    ...
    for idx, item in enumerate(candidates):
        if isinstance(item, str):
            ...
            chunk = {
                ...
                "score": (
                    item.get("score")
                    or item.get("dense_score")
                    or item.get("final_score")
                ),
                "rerank_score": (
                    item.get("rerank_score")
                    or item.get("page_score")
                    or item.get("final_score")
                ),
                "type": item.get("type") or item.get("evidence_type") or "",
            }
...
def target_function(inputs: dict) -> dict:
    ...
    if RAG_TARGET_MODE == "http":
        ...
        for attempt in range(3):
            try:
                resp = requests.post(RAG_API_URL, json=payload, timeout=240)
        ...
        return {
            "response": "",
            "rag_trace": {},
            "target_error": f"HTTP target failed: {type(last_error).__name__}: {last_error}",
        }
...
def _call_doubao_json_judge(system_prompt: str, user_prompt: str) -> dict:
    ...
    raise RuntimeError(f"Judge failed after retries: {type(last_error).__name__}: {last_error}")


# ============================================================
# 7. 检索诊断 evaluator：不调用 LLM
# ============================================================
...
def financebench_text_hit_retrieval_evaluator(run, example):
    ...
    text_hit_threshold = float(
        os.getenv("FINANCE_RAG_EVIDENCE_TEXT_HIT_THRESHOLD", "0.25")
    )
    ...
    if best_chunk:
        best_chunk_info = (
            ...
            f"Best chunk type: {best_chunk.get('type')}\n"
            ...
            f"Best retrieved text preview:\n{_truncate_text(best_chunk.get('text', ''), 1200)}"
        )
    ...
    comment = (
        ...
        f"Retrieved chunks/evidence count: {len(retrieved_chunks)}\n\n"
        f"Top retrieved evidence/chunks:\n"
...
def financebench_combined_llm_evaluator(run, example):
    ...
    if not rag_answer or _is_refusal_answer(rag_answer):
        comment = (
            "Judge result: incorrect\n\n"
            "Error type: missing_answer\n\n"
            "Reason: RAG 系统返回空答案或拒答。\n\n"
            f"Question:\n{question}\n\n"
            f"Reference answer:\n{reference_answer}\n\n"
            f"RAG answer:\n{rag_answer}\n\n"
        ...
        return [
            ...
            {"key": "fb_judge_confidence", "score": 1, "comment": "Empty/refusal RAG answer."},
...
if __name__ == "__main__":
    ...
    print("RAG_TARGET_MODE:", RAG_TARGET_MODE)
    print("RAG_API_URL:", RAG_API_URL if RAG_TARGET_MODE == "http" else "(not used)")
