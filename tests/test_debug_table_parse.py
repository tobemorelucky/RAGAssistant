...
def test_parse_args_defaults():
    ...
    assert args.show_normalized is False


def test_parse_args_override_backend_and_docling_options():
    args = debug_table_parse.parse_args(
        ["demo.pdf", "--backend", "pdfplumber_words", "--docling-ocr", "--timeout-seconds", "30", "--max-pages", "2", "--show-rejected", "--show-normalized"]
    )
    ...
    assert args.show_normalized is True
...
def test_build_report_contains_expected_fields():
        ...
        [
            {
                ...
                "normalized_columns": ["Metric", "FY2022"],
                "normalized_rows": [{"Metric": "Revenue", "FY2022": "100"}],
                "normalization_notes": [],
            }
        ],
        max_tables=5,
    ...
    assert "normalized_title: Summary" in report
    assert "normalization_level: full" in report
